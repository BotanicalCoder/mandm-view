        ...
        <div class="px-0 flex flex-col gap-3">
          ...
          <div class="flex flex-col gap-3 mt-4" v-if="targetSavings.length > 0">
            <div class="flex flex-col p-3 border bg-[#ffffff] text-black rounded-lg"
              v-for="(savingsData, index) in targetSavings" @click="async () =>
              ...
              <div class="grid grid-cols-2 gap-2">
                ...
                <div class="flex flex-col">
                  <p> Target </p>
                  <h4 class="text-base font-medium">{{ savingsData.save_target }}
                  </h4>
                </div>
          ...
          <div class="h-full my-[6rem] flex justify-center items-center" v-if="targetSavings.length == 0">
...
const { data, pending, error } = await useAsyncData<RootObj>(
  'all-target-savings',
  () => $fetch("https://kams.kolomoni.ng/api/savings/view-target-savings", {
    method: "get",
    headers: {
...
function removeDuplicates(data: any) {
  ...
  let allData = [...data, ...targetSavings.value];
