      ...
      <div class="flex flex-col justify-center p-4 gap-2 text-[#003399]" v-else>
        ...
        <div class="px-0 flex flex-col gap-3">
          <div class="flex flex-row flex-wrap justify-between mt-4">
            <button class="border bg-[#3861b4] text-[#ffffff] rounded-lg w-fit p-2 flex items-center gap-2 font-bold mb-0"
            ...
            <button class="border bg-[#3861b4] text-[#ffffff] rounded-lg w-fit p-2 flex items-center gap-2 font-bold mb-0"
              @click="async () => navigateTo('/savings/transact-n-save/transactions')
                ">
          ...
          <div class="flex flex-col gap-3 mt-4" v-if="transactNSave.length > 0">
            <div class="flex flex-col p-3 border bg-[#ffffff] text-black rounded-lg"
              v-for="(savingsData, index) in transactNSave" @click="async () =>
                await navigateTo('/savings/transact-n-save/' + savingsData.id)
                " :key="savingsData.id">
              <div class="grid grid-cols-2 gap-2">
                <div class="flex flex-col col-span-2 text-lg">
                  <h4 class="font-bold capitalize">{{ savingsData.save_label }}
                  </h4>
                </div>
          ...
          <div class="h-full my-[6rem] flex justify-center items-center" v-if="transactNSave.length == 0">
...
const { data, pending, error } = await useAsyncData<RootObj>(
  'all-transact-n-save',
  () => $fetch("https://kams.kolomoni.ng/api/savings/view-transact-and-save", {
    method: "get",
    headers: {
...
function removeDuplicates(data: any) {
  ...
  let allData = [...data, ...transactNSave.value];
